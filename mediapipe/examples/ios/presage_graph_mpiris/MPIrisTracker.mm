...
// static const char *kOutputStream = "segmentation_mask_cpu";
static const char *kOutputStream = "output_video"; //"segmentation_mask"; //
// static const char *kLandmarksOutputStream = "iris_landmarks";  // aya removed iris
...
static const char *kPoseSegmaskOutputStream = "segmentation_mask"; //_cpu";
...
@property(nonatomic) MyCameraInputSource *cameraSource;
...
- (instancetype)init {
    ...
    if (self) {
        ...
        [self.mediapipeGraph addFrameOutputStream:kPoseSegmaskOutputStream outputPacketType:MPPPacketTypePixelBuffer];

        self.mediapipeGraph.delegate = self;

        // [self.mediapipeGraph addFrameOutputStream:kLandmarksOutputStream outputPacketType:MPPPacketTypeRaw]; // aya removed iris
...
- (void)start {
    self.cameraSource = [[MyCameraInputSource alloc] init];
...
- (void)mediapipeGraph:(MPPGraph *)graph
  didOutputPixelBuffer:(CVPixelBufferRef)pixelBuffer
    ...
    if (streamName == kOutputStream) {
      NSLog(@"Frame did update: output stream");
        [_delegate frameDidUpdate:self didOutputPixelBuffer:pixelBuffer];
    }
    if (streamName == kPoseSegmaskOutputStream) {
      NSLog(@"Frame did update: segmentation mask stream");
        [_delegate frameDidUpdateSegmask:self didOutputPixelBuffer:pixelBuffer];
    }


    // todo: include segmentation mask stream here..
}
...
- (void)mediapipeGraph:(MPPGraph *)graph
    ...
    //     NSLog(@"[TS:%lld] Packet recieved kLandmarksOutputStream", packet.Timestamp().Value());
    //     if (packet.IsEmpty()) {
    ...
    if (streamName == kFaceLandmarksOutputStream) {
        NSLog(@"[TS:%i] Packet recieved FaceLandmarks OutputStream", packet.Timestamp().Value());
        if (packet.IsEmpty()) {
            NSLog(@"[TS:%i] No face landmarks", packet.Timestamp().Value());
            return;
        }
        const auto
    ...
    if (streamName == kPoseLandmarksOutputStream) {
        NSLog(@"[TS:%i] Packet recieved PoseLandmarks OutputStream", packet.Timestamp().Value());
        if (packet.IsEmpty()) {
            NSLog(@"[TS:%i] No pose landmarks", packet.Timestamp().Value());
            return;
        }
        const auto
        &pose_landmarks = packet.Get<::mediapipe::NormalizedLandmarkList>();
        //float width = 1080.0; // todo: automate this? or put it in ios
        //float height = 1920.0;
        NSMutableArray<MPLandmark *> *result = [NSMutableArray array];
        for (int i = 0; i < pose_landmarks.landmark_size(); ++i) {
            // NSLog(@"POSE LANDMARKS FOUND YESSSSS -> [%lf, %lf, %lf]", pose_landmarks.landmark(i).x(), pose_landmarks.landmark(i).y(), pose_landmarks.landmark(i).z());
            MPLandmark *landmark = [[MPLandmark alloc] initWithX:(pose_landmarks.landmark(i).x())
...
//    if (streamName == kPoseSegmaskOutputStream) {
//        NSLog(@"[TS:%i] Packet recieved PoseSegmask OutputStream!!", packet.Timestamp().Value());
//        if (packet.IsEmpty()) {
//            NSLog(@"[TS:%i] No segmentation mask", packet.Timestamp().Value());
//            return;
//        }
//    }
}
